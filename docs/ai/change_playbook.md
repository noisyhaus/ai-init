...
## Dogfooding Boundary Change
- Inspect:
  - root development docs: `AGENTS.md`, `docs/ai/*`
  - distributed templates: `templates/*`
  - distributed skill source: `skills/*/SKILL.md`
  - local runtime skill copies only when testing installed behavior: `~/.codex/skills/*`
- Common misses:
  - copying repo-specific operating rules into generated user-project templates
  - editing installed local skills but forgetting to update `skills/` in the repo
  - running `ai-init --force` in the repository root and overwriting development docs
- Required verification:
  - run scaffold tests in a temporary directory
  - confirm root docs and `templates/` changes are intentionally different or intentionally aligned
  - verify no private/local-only references leaked into public `skills/` or `templates/`
