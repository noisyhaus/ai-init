...
`ai-init`은 Codex 프로젝트를 위한 markdown-first 부트스트랩 시스템입니다.

이 저장소는 세 가지를 함께 제공합니다.

- 설치 가능한 Codex skill 하나: `ai-init`
- 로컬 scaffold 명령 하나: `ai-init`
- 여러 세션에 걸친 AI 코딩을 위한 durable project-memory layout 하나
...
이 스크립트는 로컬에 두 개의 install link를 만듭니다.
...
초기화하려는 프로젝트에서 Codex를 시작한 뒤 skill entrypoint를 사용합니다.

```text
$ai-init
```

설치된 skill이 로컬 `ai-init` 명령을 대신 호출합니다. skill은 Codex 진입점이고, command는 고정된 scaffold 생성기입니다.
