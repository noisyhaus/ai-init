...
`ai-init`은 AI 코딩 세션을 몇 번만 진지하게 돌려도 반복해서 생기는 불편함 때문에 만들어졌습니다.

- 중요한 결정이 chat 안에만 있어서 다음 session의 agent가 프로젝트 맥락을 잊습니다.
- 매번 새 session을 시작할 때 같은 설명을 반복해야 합니다.
- 기능 요청이 scope, risk, verification 없이 바로 code로 넘어갑니다.
- bugfix가 실제 재현이 아니라 추측에서 시작됩니다.
- session handoff가 "끝났다"는 말만 있고, 다음 agent가 믿을 수 있는 Git evidence가 부족합니다.
- 프로젝트 규칙, 사용자 선호, 현재 상태가 여러 note와 대화에 흩어져 drift됩니다.

`ai-init`은 이런 불안정한 습관을 project-local onboarding system으로 바꿉니다. agent가 읽고, 갱신하고, handoff할 수 있는 예측 가능한 markdown file set을 제공합니다.
...
대상 프로젝트에서 `$ai-init`을 실행하면 [templates/AGENTS.md](templates/AGENTS.md)를 바탕으로 짧고 실무적인 `AGENTS.md`가 생성됩니다. 이 파일은 Codex가 해당 프로젝트에서 따르는 project-level operating contract입니다.

생성되는 `AGENTS.md`는 agent에게 다음을 지시합니다.

- `AGENTS.md`를 두 번째 README처럼 길게 쓰지 않고, 짧고 operational하게 유지합니다.
- 시작할 때 `docs/ai/project_memory.md`, `docs/ai/change_playbook.md`, `docs/ai/current_state.md`를 읽습니다.
- markdown docs를 source of truth로 취급하고, optional recall/index file은 다시 만들 수 있는 helper로 취급합니다.
- broad refactor, speculative abstraction, silent ambiguity resolution, unrelated cleanup을 피합니다.
- feature work는 spec과 plan을 먼저 만들고, bugfix는 reproduce-diagnose-fix-verify 순서로 진행하며, refactor는 regression protection 이후에 진행합니다.
- 완료를 주장하기 전에 검증하고 concrete evidence를 보고합니다.
- session close 때는 먼저 Git 상태를 확인한 뒤 `docs/ai/current_state.md`를 갱신하고, 필요하면 handoff note를 남깁니다.

즉 생성된 `AGENTS.md`는 빈 repository나 기존 repository를 Codex가 바로 onboarding된 workspace처럼 다루게 만듭니다. agent는 무엇을 먼저 읽어야 하는지, durable memory가 어디에 있어야 하는지, 어떻게 작업을 시작하고 닫아야 하는지 알게 됩니다.
...
`$ai-init`은 프로젝트마다 한 번 실행해 onboarding scaffold를 만듭니다. 그 이후의 기본 daily loop는 context 복구, 하나의 집중 작업 lane, session close입니다. branch를 stage, commit, push, merge할 준비가 되었을 때 `$ai-init-finish-work`를 실행합니다.
