...
- This repository dogfoods `ai-init`: root `AGENTS.md` and `docs/ai/*` govern development of `ai-init` itself.
- Distributed user-project content lives under `templates/`; distributed Codex skill source lives under `skills/`.
...
- Do not run `ai-init --force` in this repository root; it can overwrite development docs with generated templates.
- Test scaffold behavior in a temporary directory, not in the repository root.
- Do not treat installed local skills under `~/.codex/skills` as the source of truth for release changes; update `skills/` in this repo first.
